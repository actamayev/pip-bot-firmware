...
class StraightLineDrive : public Singleton<StraightLineDrive> {
  ...
  public:
    void enable();
    void disable();
    void update(int16_t& left_speed, int16_t& right_speed);
    bool is_enabled() const {
        return _straightDrivingEnabled;
    }
  ...
  private:
    ...
    // Helper methods
    static float calculate_heading_error(float current_heading, float target_heading);
