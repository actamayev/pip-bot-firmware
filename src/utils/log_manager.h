// TODO 5/5/25: This log manager is blocking Do not use it
#pragma once
...
#define LOG_QUEUE_SIZE 30  // Increased size
...
class LogManager : public Singleton<LogManager> {
    ...
    private:
        ...
        // Send a message to the log queue - completely non-blocking
        bool sendToLogQueue(const char* message) {
            if (logQueue == NULL || !loggingEnabled) return false;
            ...
            // Send to queue with NO timeout - never blocks
            return xQueueSend(logQueue, &logMessage, 0) == pdTRUE;
        }
    ...
    public:
        ...
        bool isQueueNearlyFull() {
            if (logQueue == NULL) return true;
            return uxQueueSpacesAvailable(logQueue) < 5; // Arbitrary threshold
        }
        ...
        void logTofGrid(VL53L7CX_ResultsData* data, float avgDistance, bool objectDetected) {
            // Skip logging if disabled or queue is getting full
            if (!loggingEnabled || isQueueNearlyFull()) return;
